...
public class IntBigTorusKnotTests {
	...
	private static final String T76_zmodule = "q^29*t^0*Z[0] + q^31*t^0*Z[0] + q^33*t^2*Z[0] + q^37*t^3*Z[0] + q^35*t^4*Z[0] + q^37*t^4*Z[0] + q^39*t^5*Z[0] + q^41*t^5*Z[0] + q^37*t^6*Z[0] + q^39*t^6*Z[0] + q^41*t^7*Z[0] + q^43*t^7*Z[0] + q^39*t^8*Z[0] + q^41*t^8*Z[0,0] + q^43*t^9*Z[0] + q^45*t^9*Z[0,0] + q^41*t^10*Z[0] + q^43*t^10*Z[0,0] + q^45*t^11*Z[0] + q^47*t^11*Z[0,0,0] + q^45*t^12*Z[0,0] + q^47*t^12*Z[0] + q^51*t^12*Z[0] + q^49*t^13*Z[0,0,0] + q^51*t^13*Z[0] + q^47*t^14*Z[0] + q^49*t^14*Z[0] + q^53*t^14*Z[0] + q^51*t^15*Z[0,0] + q^53*t^15*Z[0,0] + q^49*t^16*Z[0] + q^51*t^16*Z[0] + q^55*t^16*Z[0] + q^57*t^16*Z[0] + q^53*t^17*Z[0] + q^55*t^17*Z[0] + q^53*t^18*Z[0] + q^57*t^19*Z[0]";
